# ⏳ Reflexion: Fokussierte Stunden – Deine 3 mächtigen Menüs

## 🥜 In 30 Sekunden

- **5-50-5 Formel**: Vorbereiten → Fokussieren → Reflektieren
- **3 Menüs**: Aktivierung, Reaktivierung, Erfassung
- **4 Reiter des Fokus-Tods**: Prokrastination, Ablenkung, Burnout, Perfektionismus
- **Fokus-Logbuch**: Tracke täglich deine Fokusminuten
- **Menü-Beispiele**: Konkrete Strategien für jede Situation

## ⏳ Die Fokus-Formel: 5-50-5

Im Zentrum eines sinnvollen Fortschritts steht die Fähigkeit, sich intensiv auf wichtige Arbeit zu konzentrieren.

### Die ⏳ Formel für fokussierte Stunden

Ein strukturierter Ansatz für konzentriertes Arbeiten:

**5 Min** → Vorbereitung (Was will ich erreichen?)
**50 Min** → Fokussieren (Deep Work, keine Ablenkungen)
**5 Min** → Nachdenken & Auftanken (Reflexion + Pause)

## 🚫 Die 4 Reiter des Fokus-Tods

Wenn du diese Praxis beherrschst und lernst, die **„4 Reiter"** zu bekämpfen, schaffst du die Grundlage für jedes Ziel:

1. **Aufschieberitis** (Prokrastination) → Lösung: Aktivierungsmenü
2. **Ablenkung** → Lösung: Reaktivierungsmenü
3. **Burnout** → Lösung: Pausen + Regenerieren
4. **Perfektionismus** → Lösung: "Fortschritt > Perfektion"

## ⏰ Das Fokus-Logbuch: Messen ist Macht

In vielerlei Hinsicht ist das wirksamste Mittel zur Verbesserung deiner Konzentration, **zu verfolgen, wie lange du tatsächlich mit tiefer, konzentrierter Arbeit verbringst**.

Nimm dir die nächsten Arbeitstage Zeit, um deine Fokuszeit zu erfassen (nicht für Verwaltung, Besprechungen oder Ablenkungen), um die Bedingungen, Tageszeiten und Projekte zu ermitteln, auf die du dich am besten konzentrieren kannst.

**Hinweis:** Du kannst deine Fokusminuten schriftlich erfassen oder die kostenlose App [toggl.com](https://toggl.com) verwenden.

### Einfache Tracking-Methode:

```
[Datum + Uhrzeit] - Aufgabe - Fokusreflexion
[Datum + Uhrzeit] - Aufgabe - Fokusreflexion
[Datum + Uhrzeit] - Aufgabe - Fokusreflexion
```

### Beispiel:

**3. März 2025, 09:30 - 10:15 Uhr - Outline für Blogpost erstellen**
*Reflexion:* Sehr effizient gearbeitet. Nur das Nötigste gemacht und dennoch zufriedenstellendes Ergebnis erzielt. Gute Balance zwischen Aufwand und Ergebnis gefunden. Anschließend kurze Pause genommen.

**3. März 2025, 10:30 - 11:45 Uhr - Blogpost vervollständigen**
*Reflexion:* Konzentrationsniveau etwas niedriger als bei vorheriger Aufgabe, aber dennoch produktiv. Workflow war gut strukturiert - Text fertiggestellt und mit einmaligem Korrekturlesen abgeschlossen. Effizienter Arbeitsprozess.

**3. März 2025, 13:00 - 14:30 Uhr - Podcast editieren**
*Reflexion:* Angenehme Arbeitsphase. Systematisch Schritt für Schritt vorgegangen. Diese fokussierte Herangehensweise war deutlich effektiver als das sonst übliche gestresste Hin-und-her-Springen zwischen Aufgaben.

## ⚡️ Deine 3 Aktivierungs-Menüs

### ↗️ **1. Aktivierungsmenü: Die „Einstiegsrampe" zum Fokus**

Eine Möglichkeit, **Prokrastination** zu bekämpfen, ist die Erstellung eines sogenannten **Aktivierungsmenüs**: eine Liste von Dingen, die dir helfen, anzufangen – sozusagen eine "Startrampe" um in den Flow zu kommen.

**Die Idee:** Vervollständige diesen Satz:

*"Wenn ich bemerke, dass ich prokrastiniere, werde ich eines oder mehrere der folgenden Dinge tun..."*

**Beispiel-Menü:**
- [ ] 5 tiefe Atemzüge nehmen
- [ ] Schreibtisch aufräumen
- [ ] 2-Minuten-Version der Aufgabe starten
- [ ] Fokus-Musik anmachen
- [ ] Smartphone in anderen Raum legen
- [ ] Timer auf 10 Min stellen ("Ich arbeite nur 10 Min")

**💪 Übung: Erstelle dein eigenes Aktivierungsmenü**

Liste 5-7 schnelle Dinge auf, die dir helfen, auf die "Auffahrt" zum Fokusmodus zu kommen. Halte diese Liste auf deinem Arbeitsplatz sichtbar.

### 🔁 **2. Reaktivierungsmenü: Wieder auf Kurs kommen**

Ähnlich verhält es sich, wenn du **während einer Fokussitzung abgelenkt** wirst. Es hilft, ein **Reaktivierungsmenü** zu haben, um wieder auf Kurs zu kommen.

**Vervollständige diesen Satz:**

*"Wenn ich feststelle, dass ich mich während des Fokussierens ablenken lasse, werde ich eines oder mehrere der folgenden Dinge tun..."*

**Beispiel-Menü:**
- [ ] 3 tiefe Atemzüge
- [ ] Augen schließen für 10 Sekunden
- [ ] Aufstehen, kurz strecken
- [ ] Ziel der Fokusphase nochmal lesen
- [ ] Ablenkende Tab schließen
- [ ] Gedanken ins Erfassungs-Notizbuch schreiben

**💪 Übung: Erstelle dein eigenes Reaktivierungsmenü**

Liste 5-7 schnelle Dinge auf, die dir helfen, wieder in den Fokusmodus zurückzukehren. Halte diese Liste auf deinem Arbeitsplatz sichtbar.

### 🪝 **3. Erfassungsmethode: Ein "Parkplatz" für zufällige Gedanken**

Für zufällige Gedanken, die während Fokussitzungen auftauchen, halte ich ein kleines **Notizbuch** neben mir. Auf diese Weise kannst du Dinge schnell notieren, ohne deine Konzentration zu stören.

Früher habe ich versucht, mir alles zu merken, aber ich habe festgestellt, dass das Festhalten an diesen Gedanken es tatsächlich **schwieriger** machte, sich zu konzentrieren.

Ein festgelegter **"Parkplatz"** für zufällige Ideen hilft dir, den Fluss aufrechtzuerhalten und gibt dir die Gewissheit, dass nichts Wichtiges vergessen wird.

**Beispiele:**
- Notizbuch neben dem Laptop
- Notion "Inbox" Seite offen
- Things "Quick Entry" Shortcut
- Einfaches Blatt Papier

**💪 Übung: Erstelle deine eigene Erfassungsmethode**

Ob digital (z.B. Apps wie Things) oder analog (z.B. Notizbuch), entwickle eine Erfassungsmethode für zufällige Gedanken und Aufgaben, die dir während einer Fokussitzung in den Sinn kommen.

## ✅ Deine nächsten Schritte

- [ ] **Aktivierungsmenü** erstellen (5-7 Punkte)
- [ ] **Reaktivierungsmenü** erstellen (5-7 Punkte)
- [ ] **Erfassungsmethode** einrichten (Notizbuch oder App)
- [ ] **Fokus-Logbuch** führen (täglich tracken)
- [ ] **Menüs sichtbar** machen (am Arbeitsplatz)

## 🎯 Die wichtigste Erkenntnis

**Fokus ist keine Willenskraft-Frage.**

**Fokus ist eine System-Frage.**

Mit den richtigen **Menüs** (Aktivierung, Reaktivierung, Erfassung) und dem **Fokus-Logbuch** baust du ein System, das funktioniert – auch an schwierigen Tagen.

**→ Weiter geht's mit "Experiment Woche 2"**
